...
export default function GarageLayout({ children }: { children: React.ReactNode }) {
  const [userName, setUserName] = useState('פרונט ראשון');
    ...
    fetch('/api/auth/me')
      ...
      .then(data => {
        ...
        if (data?.user?.role) {
          ...
          if (data.user.role !== 'garage_owner' && data.user.role !== 'admin') {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <Sidebar portal="garage" userName={userName} userRole={userRole} />
